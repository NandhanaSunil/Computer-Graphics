...
Sphere::Sphere(float radius, int sectorCount, int stackCount) {
  ...
  glVertexAttribPointer(3, 2, GL_FLOAT, GL_FALSE, 2 * sizeof(float), (void*)0); // Texture coordinates
...
void Sphere::generateSphere(float radius, int sectorCount, int stackCount) {
  ...
  for (int i = 0; i <= stackCount; ++i) {
      ...
      for (int j = 0; j <= sectorCount; ++j) {
          ...
          float u = (((float)j / sectorCount)); // Invert horizontal mapping if needed
          float v = ((float)i / stackCount); // Use the corrected latitude mapping
